...
def collate_masks(durations): #mask自己
...
def collate_masks2(durations): #前1后1
  """Convert a list of 1d tensors into a padded 2d tensor."""
  out_lens = durations.sum(dim=1)
  max_len = out_lens.max()
  bsz, seq_len = durations.size()
...
def collate_masks2qian0hou1(durations):
  """Convert a list of 1d tensors into a padded 2d tensor."""
  out_lens = durations.sum(dim=1)
  max_len = out_lens.max()
  bsz, seq_len = durations.size()
...
def collate_masks3qian2hou2(durations):  # 前2后2  最后一行有点问题 时间关系 没修改 直接用了
  """Convert a list of 1d tensors into a padded 2d tensor."""
  num = 2  # 后面mask数
  num2=2
  out_lens = durations.sum(dim=1)  # [6,5]
...
def collate_masks3qian1hou0(durations):  # 前1后0
  """Convert a list of 1d tensors into a padded 2d tensor."""
  num = 0  # 后面mask数
  # num2=1
  out_lens = durations.sum(dim=1)  # [6,5]
