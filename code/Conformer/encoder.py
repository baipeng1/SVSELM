...
from Conformer.vgg2l import VGG2L
from Conformer.attention import (
    LegacyRelPositionMultiHeadedAttention,
    MultiHeadedAttention,
    RelPositionMultiHeadedAttention,
    RelPositionMultiHeadedAttentionlocal,
)
...
from Conformer.layer_norm import LayerNorm
from Conformer.multi_layer_conv import (
    Conv1dLinear,
    MultiLayeredConv1d,
)
from Conformer.positionwise_feed_forward import (
    PositionwiseFeedForward,
)
from Conformer.repeat import repeat
from Conformer.subsampling import Conv2dSubsampling
...
class Encoder(torch.nn.Module):
    ...
    def __init__(
        ...
        positional_dropout_rate=0.2, #0.1
        attention_dropout_rate=0.2, #0.0
        input_layer=None,  #conv2d
        normalize_before=True,
        concat_after=False,
        positionwise_layer_type="conv1d",  #"linear"
        positionwise_conv_kernel_size=3,  #1
        macaron_style=True,
        pos_enc_layer_type="rel_pos",  #abs_pos
        selfattention_layer_type="rel_selfattn",
        activation_type="swish",
        use_cnn_module=True,  #False
        zero_triu=False,
        ...
        if selfattention_layer_type == "selfattn":
            ...
            encoder_selfattn_layer = RelPositionMultiHeadedAttentionlocal
            encoder_selfattn_layer_args = (
                attention_heads,
                attention_dim,
                attention_dropout_rate,
    ...
    def forward(self, xs ,atten_mask=None,masks=None):
        ...
        """
        masks = xs.abs().sum(-1).eq(0).data if masks is None else masks
        ...
        if self.intermediate_layers is None:
            xs, _, masks = self.encoders(xs, atten_mask, masks)
        else:
            intermediate_outputs = []
            for layer_idx, encoder_layer in enumerate(self.encoders):
                xs, masks = encoder_layer(xs, atten_mask,masks)
