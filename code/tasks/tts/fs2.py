...
from utils.foacal_losszuizhong import frame2phdur
...
class FastSpeech2Task(TtsTask):
    ...
    def validation_step(self, sample, batch_idx):
        outputs = {}
        outputs['losses'] = {}
        outputs['losses'], model_out = self.run_model2(self.model, sample, return_output=True)
...
#训练损失
    def run_model(self, model, sample, return_output=False):
        ...
        attentionmaskjuzhen = sample['attentionmaskjuzhen']

        output = model(txt_tokens, atten_mask=attentionmaskjuzhen,mel2ph=mel2ph, spk_embed=spk_embed,
                       ref_mels=target, f0=f0, uv=uv, energy=energy, infer=False)

        losses = {}
        # self.add_mel_loss(output['mel_out'], target, losses)
        phdur = frame2phdur(mel2ph)
        focalloss = self.compute_w(output['mel_out'], phdur, target, wd=1, alpha=2)
        focal_loss = {'focal_loss': focalloss}
        losses.update(focal_loss)
        # ssimloss = self.ssim_loss(output['mel_out'], target)
        # ssim_loss = {'ssim_loss': ssimloss}
        # losses.update(ssim_loss)
...
#验证损失
    def run_model2(self, model, sample, return_output=False):
        txt_tokens = sample['txt_tokens']  # [B, T_t]
        target = sample['mels']  # [B, T_s, 80]
        mel2ph = sample['mel2ph']  # [B, T_s]
        f0 = sample['f0']
        ...
        self.add_mel_loss(output['mel_out'], target, losses)
        #只要l1验证
        # l1loss = self.l1_loss(output['mel_out'], target)
        # l1_loss = {'l1_loss': l1loss}
        # losses.update(l1_loss)
        self.add_dur_loss(output['dur'], mel2ph, txt_tokens, losses=losses)
        if hparams['use_pitch_embed']:
            self.add_pitch_loss(output, sample, losses)
        if hparams['use_energy_embed']:
...
#focal loss��ʼ
    def compute_w(self, _feat_out, phduration, target, wd=0.05, alpha=2):
        w_loss = F.l1_loss(_feat_out, target, reduction='none') #����l1loss 1x1953x80 [B ,T 80]
        bsz, seq_len, dim = _feat_out.size() #batch_size[B]���������� ֡�� seq_len[T] dim=80 Ҳ����1 1953  80
        durations = phduration #[B,音素时长数量]，eg[18,91]就是18个样本，时长含有音素时长数组最大是91，也都得扩充成91，不够的后面补0���ؼ���ʱ�� [1,68] �洢ÿһ�����ض�Ӧ����֡
        _, src_l = durations.size() #src_l就是音素时长数量 �����ظ��� src_l=68
        out_lens = durations.sum(dim=1) #out_lens维度是[18]，每一个元素表示的是每一个样本的总帧数[1999,1898,^,1999]18个这样的数�����֡���� 1953
        out = torch.zeros(bsz, src_l).to(_feat_out.device) #[B,音素时长数]比如[18,91][batch_size, ���س���] ��1��68�����Ҫ��ż����Ȩ�� [B,]
        for b in range(bsz): #ÿһ������
            l = 0
            indices = []
            for t in range(src_l):  #ÿһ������
                step = durations[b, t]
                if step == 0:
                    continue
                a = w_loss[b, l:l + durations[b, t], :] #����ȡÿһ�����ز��ֵ���ʧ��������ʽ��ÿһ�����������Ӧ��֡����80������[83��80] [24,80]
                ph_ave = torch.mean(a) #ÿһ�����ص�80mel��ƽ��ֵ
                l = l + step
                out[b, t] = ph_ave
        #out [1,68] ������x������
        mask = out == 0 #out中元素为0的给到mask中为true��Ԫ��Ϊ0����true,��0Ϊfalse [1,68] ������x������
        out2 = out.masked_fill(mask, float("-inf"))  #[18,91]out中按照mask中真假值填充，其实就是为0的为true了，然后填充上负无穷大，out2完了就是一个前面有数值，后面是负无穷大的矩阵��true���ָ�Ϊ-�����Ҳ���������Щû�����ص� [1,68] ������x������
        ph_w = F.softmax(out2 / wd, dim=1)  #���ؼ���Ȩ��
        ph_w2 = ph_w.clone().detach()  #ph_w就是softmax后的权重系数矩阵维度也是[18,91] 18是B 91是时长的数量
        # zhi shu fang da
        # ph_w = ph_w**alpha
        # ph_w = F.softmax(out2, dim=1)

        exp_ph_w, ll = self.expand(ph_w2, durations) #exp_ph_w 维度是[18,1998]也就是【B，帧数】 ll其实是【18】 每个里面存的帧数��1x1953��Ҳ����������x֡��  ll����֡�� ����Ҳû��
        N = torch.numel(_feat_out) #Tensorһ������Ԫ�� N为18x1998x80 N是左右样本的格子数 像素数
        l1_loss = (w_loss * exp_ph_w.unsqueeze(-1)).sum() / N #w_loss是【BTC】exp_ph_w是【BT】unsqueeze(-1)一下后，变为【BTC】exp_ph_w.unsqueeze(-1) ���ǽ�֡�����Ȩ�ز�ֿ�1953����һ����ȥ��ÿһ��֡�����Ƶ��ֵ
        return l1_loss #这就是所有样本像素数的L1损失，最终转为一个数

#����ǳ����������������ֵ��һ��������
    def compute_wcvpr(self, _feat_out, target):
        w_loss = F.l1_loss(_feat_out, target, reduction='none') #����l1loss 1x1953x80  batch_sizex֡��x80
        weightjuzhen = w_loss / w_loss.max()
        weightjuzhen = torch.clamp(weightjuzhen,min=0.0,max=1.0)
        weightjuzhen2 = weightjuzhen.clone().detach()
        N = torch.numel(_feat_out)
        focal_loss = (weightjuzhen2 * w_loss).sum() / N
        return focal_loss

    #g��Ϊȡÿһ�����������ֵ��һ�����ټ���ģ����������
    def compute_wcvpr2(self, _feat_out, target):
        w_loss = F.l1_loss(_feat_out, target, reduction='none') #����l1loss 1x1953x80  batch_sizex֡��x80
        weightjuzhen = torch.zeros_like(w_loss)  # ��w_lossһ�����Ȩ�ؾ���
        for b in range(w_loss.shape[0]): #b��ʾÿһ������
            weightjuzhen[b] = w_loss[b] / w_loss[b].max()
            weightjuzhen[b] = torch.clamp(weightjuzhen[b],min=0.0,max=1.0)
        weightjuzhen2 = weightjuzhen.clone().detach()
        N = torch.numel(_feat_out)
        focal_loss = (weightjuzhen2 * w_loss).sum() / N
        return focal_loss
...
#ÿһ֡����Ȩ�صķ���
    def compute_wframe(self, _feat_out, target):
        w_loss = F.l1_loss(_feat_out, target, reduction='none') #����l1loss 1x1953x80
        quanzhongjuzhen = torch.zeros_like(w_loss) #��w_lossһ�����Ȩ�ؾ���
        for b in range(w_loss.shape[0]): #ÿһ������
            #����ϲ�
            w_losshengxiang = torch.sum(w_loss[b],dim=0)
            quaznhongmelbins = F.normalize(w_losshengxiang,p=1,dim=0) #���� 80
            #����ϲ�
            w_losszongxiang = torch.sum(w_loss[b],dim=1)
            quanzhongframe = F.normalize(w_losszongxiang,p=1,dim=0) #���� ֡
            quanzhongjuzhen[b] = quanzhongframe.reshape(-1,1)*quaznhongmelbins  #����ÿһ��������Ȩ�ؾ���
        quanzhongjuzhen2 = quanzhongjuzhen.clone().detach()
        N = torch.numel(_feat_out)
        focal_loss = (quanzhongjuzhen2 * w_loss).sum() / N
        return focal_loss
    ...
    def expand(self, x, durations):
        ...
        for b in range(bsz):
            ...
            for t in range(seq_len):
                indices.extend([t] * durations[b, t])
    ...
    def test_step(self, sample, batch_idx):
        ...
        if hparams['profile_infer']:
            ...
            if hparams.get('use_midi') is not None and hparams['use_midi']:
                outputs = self.model(txt_tokens, atten_mask=attentionmaskjuzhen, spk_embed=spk_embed, mel2ph=mel2ph, f0=f0, uv=uv, ref_mels=ref_mels, infer=True,
                ...
                outputs = self.model(
                    txt_tokens, atten_mask=attentionmaskjuzhen, spk_embed=spk_embed, mel2ph=mel2ph, f0=f0, uv=uv, ref_mels=ref_mels, infer=True)
                # outputs = self.model(
                #     txt_tokens, spk_embed=spk_embed, mel2ph=mel2ph, f0=f0, uv=uv, ref_mels=ref_mels, infer=True)
