...
FS_DECODERS = {
    ...
    'conformer': lambda hp: Encoder(
        # idim = hp['hidden_size'],num_blocks = hp['dec_layers'],positionwise_conv_kernel_size=1,attention_heads=4),
        idim = 0,num_blocks = hp['dec_layers'],positionwise_conv_kernel_size=1,attention_heads=4),
}
...
class FastSpeech2(nn.Module):
    ...
    def forward(self, txt_tokens, atten_mask=None,mel2ph=None, spk_embed=None,
        ...
        if hparams['use_pitch_embed']:
            ...
            aaa=self.add_pitch(pitch_inp, f0, uv, mel2ph, ret, encoder_out=pitch_inp_ph)
            decoder_inp = decoder_inp + aaa
        if hparams['use_energy_embed']:
        ...
        if skip_decoder:
            return ret
        # masks = decoder_inp.abs().sum(-1).eq(0).data
        # masks = masks.unsqueeze(1)
        ret['mel_out'] = self.run_decoder(decoder_inp, atten_mask,tgt_nonpadding, ret, infer=infer, **kwargs)
    ...
    def run_decoder(self, decoder_inp, atten_mask,tgt_nonpadding, ret, infer, **kwargs):
        x = decoder_inp  # [B, T, H]
        # masks = x.abs().sum(-1).eq(0).data
        # masks = masks.unsqueeze(1)
        x = self.decoder(x,atten_mask)
        x = self.mel_out(x)
